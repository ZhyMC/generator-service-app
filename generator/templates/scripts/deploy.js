...
function createService(serv_name, image_name, expose_ports, config_name) {
    ...
    let mount_param = [];
    mount_param.push(`--mount`, `type=volume,source=${serv_name},target=/var/${serv_name}`);
    let proc = spawn(`docker`, ['service', 'create', ...mount_param, "-d", '--config', `source=${config_name},target=/etc/app.conf`, '--name', serv_name, ...ports_param, image_name]);
    proc.stdout.pipe(process.stdout)
    proc.stderr.pipe(process.stderr)
    proc.on("exit", (code) => {
        if (code != 0)
...
function runScript() {
    ...
    const expose_ports = package.config.expose_port;
    ...
    if (isDockerServiceRunning(serv_name)) {
        ...
        createService(serv_name, image_name, expose_ports, config_name);
    }
